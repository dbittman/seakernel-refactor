...
QEMU_NET=-netdev socket,id=n0,connect=:1234 -device e1000,netdev=n0
test: $(BUILDDIR)/kernel.elf $(USRPROGS) $(BUILDDIR)/initrd.tar $(BUILDDIR)/hd.img
	qemu-system-$(ARCH) -m 1024 -machine $(MACHINE) $(QEMU_FLAGS) -kernel $(BUILDDIR)/kernel.elf -serial stdio -initrd $(BUILDDIR)/initrd.tar $(QEMU_AHCI) $(QEMU_NET)
