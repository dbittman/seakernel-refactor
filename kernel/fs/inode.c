...
static ssize_t _inode_proc_lru_read_entry(void *item, size_t off, size_t len, char *buf)
{
	...
	PROCFS_PRINTF(off, len, buf, current,
			"%ld(%s).%ld %c", node->id.fsid, node->fs ? node->fs->driver->name : "none", node->id.inoid,
...
__initializer static void _inode_init_lru(void)
{
	...
	kobj_lru_create(&inode_lru, sizeof(struct inode_id), 0, &kobj_inode, _inode_initialize, _inode_release, NULL, NULL);
