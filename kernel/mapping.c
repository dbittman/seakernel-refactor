...
void process_remove_mappings(struct process *proc, bool user_tls_too)
{
	...
	for(int i=0;i<MMU_NUM_PAGESIZE_LEVELS;i++) {
		...
		for(struct linkedentry *entry = linkedlist_iter_start(&proc->maps[i]);
			...
			if(!user_tls_too && reg->start >= USER_TLS_REGION_START && reg->start < USER_TLS_REGION_END)
				continue;
			
			linkedlist_remove(&proc->maps[i], entry);
...
void process_copy_mappings(struct process *from, struct process *to)
{
	...
	for(int i=0;i<MMU_NUM_PAGESIZE_LEVELS;i++) {
		for(struct linkedentry *entry = linkedlist_iter_start(&from->maps[i]);
			...
			struct map_region *reg = linkedentry_obj(entry);
			map_region_setup(to, reg->start, reg->length, reg->prot, reg->flags, reg->file, reg->nodepage, reg->psize, true);
...
static struct map_region *__split_region(struct process *proc, struct map_region *old, uintptr_t split)
{
	...
	reg->prot = old->prot;
	reg->flags = old->flags;
	reg->file = old->file ? kobj_getref(old->file) : NULL;
	reg->nodepage = old->nodepage;
	reg->start = split;
	reg->psize = old->psize;

	assert(reg->length + old->length == oldlen);
	linkedlist_insert(&proc->maps[__get_pagelevel(old->psize)], &reg->entry, reg);
...
int map_change_protect(struct process *proc, uintptr_t virt, size_t len, int prot)
{
	...
	for(uintptr_t start = virt;start < virt + len;start += arch_mm_page_size(0)) {
		...
		if((prot & PROT_WRITE) && reg->file && !(reg->file->flags & F_WRITE)) {
			mutex_release(&proc->map_lock);
			return -EACCES;
		}
		if(start != reg->start) {
...
int mmu_mappings_handle_fault(uintptr_t addr, int flags)
{
	 ...
	 *         mark writable
	 */
	//printk("%d :: PF %lx %x\n", current_thread->process->pid, addr, flags);
	bool success = false;
	...
	if(flags & FAULT_ERROR_PRES) {
		...
		if((flags & FAULT_WRITE) && (reg->flags & MMAP_MAP_PRIVATE)) {
			if(reg->flags & MMAP_MAP_ANON) {
				uintptr_t phys;
				size_t ps;
				bool r = arch_mm_virtual_getmap(proc->ctx, v, &phys, NULL, &ps);
				assert(r);
				assert(ps == reg->psize);
