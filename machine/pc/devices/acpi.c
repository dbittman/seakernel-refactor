...
static struct xsdt_desc *get_xsdt_addr(void)
{
	...
	return (void *)(rsdp->xsdt_addr + PHYS_MAP_START);
}
...
static struct rsdt_desc *get_rsdt_addr(void)
{
	...
	return (void *)(rsdp->rsdp.rsdt_addr + PHYS_MAP_START);
}
...
void *acpi_find_table(const char *sig)
{
	if(rsdp->rsdp.rev == 0) {
		...
		for(int i=0;i<entries;i++) {
			struct sdt_header *head = (void *)(rsdt->sdts[i] + PHYS_MAP_START);
		...
		for(int i=0;i<entries;i++) {
			struct sdt_header *head = (void *)(xsdt->sdts[i] + PHYS_MAP_START);
			if(!strncmp(head->sig, sig, 4))
				return head;
		}
	}
...
{
	uintptr_t ebda = *(uint16_t *)(PHYS_MAP_START + 0x40E) << 4;
	for(uintptr_t search = ebda; search < (ebda + 1024); search += 16) {
		struct rsdp_desc *desc = (void *)(search + PHYS_MAP_START);
